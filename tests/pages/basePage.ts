...
export class BasePage {
  ...
  //constructor - оперант в котором присваиваются все значения для заявленных переменных в "модели"
  constructor(page: Page, path: string) {
    //this - оперант инициализации сущности, присваивание значений к переменной.
    this.page = page;
    this.path = path;
  ...
  async upLoadFile(fileName: string) {
    await this.uploadButton.setInputFiles(path.join(__dirname + '/../', fileName));
...
test.describe('Проверки биллинга', () => {
  test('Отображение дашборда', async ({ billingControlPage }) => {
    await test.step('Клик по кнопке "Актуальность данных"', async () => {
      await billingControlPage.page
        .locator('div', { hasText: 'Контроль' })
        .getByRole('button')
        .nth(2)
        .click();
      await billingControlPage.waitForTimeout(3000);
    });

    await test.step('Видимость кнопок "Импорт"', async () => {
      await expect(billingControlPage.getButtonByRowName('Метрики')).toBeVisible();
      await expect(billingControlPage.getButtonByRowName('Заказы')).toBeVisible();
      await expect(billingControlPage.getButtonByRowName('Сайзинги')).toBeVisible();
      await expect(billingControlPage.importAllDataButton).toBeVisible();
    });
  });
});
